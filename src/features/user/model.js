...
module.exports = {
  ...
  create(data) {
    return Model.create(data).then(user =>
      Model.findById(user._id, defProjection)
        .lean()
        .exec()
    )
  },
  ...
  getAll(options) {
    const pipeLine = []
    const { supervisor, sortBy, sortOrder, page, count, search, filter = {} } = options
...
const indexes = [Model.collection.createIndex({ phone: 1 }, { unique: true, background: true })]
